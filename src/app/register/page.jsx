...
const RegisterPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (error) {
      setPasswordError(error.message || "Failed to register account.");
      return;
    }
      ...
      <div className="flex items-center justify-center px-6 py-16 bg-white">
        <div className="w-full max-w-sm">
          ...
          <p className="text-sm text-gray-500 mb-8">
            Register to start booking from the fleet.
          </p>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <label className="flex items-start gap-2 text-xs font-medium text-gray-600 cursor-pointer">
              <input type="checkbox" required className="accent-black mt-0.5" />
          ...
          <button
            onClick={signIn}
            type="button"
            className="w-full flex items-center justify-center gap-2 border border-gray-300 rounded-md py-2.5 text-sm font-medium hover:bg-gray-50 transition cursor-pointer"
